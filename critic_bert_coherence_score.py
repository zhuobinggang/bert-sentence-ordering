...
def bert_input_single_paragraph(paragraph, predicted_label):
    ordered_paragraph = [None] * 5
    for idx, label in enumerate(predicted_label):
        ordered_paragraph[label - 1] = paragraph[idx]
        
    inputs_ids = [default_tokenizer().cls_token_id]
    for sent in ordered_paragraph:
    ...
    token_type_ids = [0] * len(inputs_ids) # 只有一段，全是0
...
def train():
    model = CriticBert() # 依然输出 nn.Linear(hidden_size, 1)，去掉 Sigmoid
    model.to(DEVICE)
    model.train()
    ...
    for item in tqdm(train_set, desc="Training CriticBert"):
        ...
        # 决定目标：如果 tau1 > tau2，target 应为 1；反之为 -1
        target = 1.0 if tau1 > tau2 else -1.0
        target_tensor = torch.tensor([target], dtype=torch.float).to(DEVICE)
        
        # 1. 产生第一种排列的输入并过模型
        ids1, mask1, type1 = bert_input_single_paragraph(paragraph, predicted_label_first)
        ids1 = torch.tensor(ids1).unsqueeze(0).to(DEVICE)
        mask1 = torch.tensor(mask1).unsqueeze(0).to(DEVICE)
        type1 = torch.tensor(type1).unsqueeze(0).to(DEVICE)
        score1 = model(ids1, mask1, type1) # [1, 1]
        
        # 2. 产生第二种排列的输入并过模型
        ids2, mask2, type2 = bert_input_single_paragraph(paragraph, predicted_label_second)
        ids2 = torch.tensor(ids2).unsqueeze(0).to(DEVICE)
        mask2 = torch.tensor(mask2).unsqueeze(0).to(DEVICE)
        type2 = torch.tensor(type2).unsqueeze(0).to(DEVICE)
        score2 = model(ids2, mask2, type2) # [1, 1]
        
        # 3. 计算 Margin Loss
        loss = criterion(score1.squeeze(-1), score2.squeeze(-1), target_tensor) / 16
        loss.backward()
...
def valid_trained(model = None):
    ...
    for item in tqdm(val_set, desc="Validating CriticBert"):
        ...
        # 双保险：如果两个预测的 Tau 值一样，Critic 无法判断谁好，直接跳过
        if tau1 == tau2:
            continue
            
        label = 1 if tau1 > tau2 else 0
        
        # 1. 产生并处理第一个排列的输入（单文本输入）
        ids1, mask1, type1 = bert_input_single_paragraph(paragraph, predicted_label_first)
        ids1 = torch.tensor(ids1).unsqueeze(0).to(DEVICE) # [1, seq_len]
        mask1 = torch.tensor(mask1).unsqueeze(0).to(DEVICE) # [1, seq_len]
        type1 = torch.tensor(type1).unsqueeze(0).to(DEVICE) # [1, seq_len]
        
        # 2. 产生并处理第二个排列的输入（单文本输入）
        ids2, mask2, type2 = bert_input_single_paragraph(paragraph, predicted_label_second)
        ids2 = torch.tensor(ids2).unsqueeze(0).to(DEVICE) # [1, seq_len]
        mask2 = torch.tensor(mask2).unsqueeze(0).to(DEVICE) # [1, seq_len]
        type2 = torch.tensor(type2).unsqueeze(0).to(DEVICE) # [1, seq_len]
        
        # 3. 分别进行前向传播，得到各自的连续相干性得分
        with torch.no_grad():
            score1 = model(ids1, mask1, type1)
            score2 = model(ids2, mask2, type2)
            
        # 4. 比较两个独立打分，分数高的那个排列胜出
        predict = 1 if score1.item() > score2.item() else 0
        
        predicts.append(predict)
        labels.append(label)
        
    # 防止过滤后没有有效验证样本导致 sklearn 报错
    if len(labels) == 0:
        print("Warning: No valid validation samples available after filtering.")
        return [], []

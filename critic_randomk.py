...
def valid_bert_n_pass_coherency(sind = True, split = 'val', npass = 2):
    ...
    for file in matching_files:
        ...
        bert.to(DEVICE)
        bert.eval()
        print(f'Validating model {file} on coherency...')
        results = []
        for paragraph in tqdm(paragraphs):
            predicted_labels = [] # 存储每次解码的 predicted_labels，如果已经有了，就不再评分了
            # npass次解码
            ...
            for _ in range(npass):
                ...
                temp_mask_token_5index_logits = get_mask_token_5index_logits(bert_input.input_ids, bert_input.attention_mask, bert)
                temp_predicted_labels = hungarian_algorithm_best_order(temp_mask_token_5index_logits.cpu().numpy())
                temp_resorted_labels = resort_paragraph(random_labels, temp_predicted_labels)
                if len(predicted_labels) > 0 and not list_in(temp_resorted_labels, predicted_labels):
                    consistent = False
                    break
                else:

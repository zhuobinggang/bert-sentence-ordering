...
# 性能图改为3子图，加上neurIPS数据
def triple_draw_test():
    ...
    # NeurIPS 数据（已修正变量名）
    labels_nips = ['BERT4SO♢', 'BERSON', 'BERSON + BOID', 'Direct MLM', 'Random-2', 'Random-3', 'Random-4',
                   'BERT4SO♢ (Random-2)', 'BERT4SO♢ (Random-3)', 'BERT4SO♢ (Random-4)']
    x_nips = [1.0, 9.7, 9.7, 1.0, 4.0, 6.0, 8.0, 4.0, 6.0, 8.0]
    y_nips = [0.785, 0.850, 0.851, 0.782, 0.793, 0.799, 0.800, 0.796, 0.797, 0.803]
    ...
    def plot_dataset(ax, x_data, y_data, labels_data, dataset_name):
        ...
        # 绘制散点
        ax.scatter(x_other, y_other, color='#ff7f0e', marker='o', s=120, label='Baselines', zorder=2)
        ax.scatter(x_our, y_our, color='#1f77b4', marker='^', s=120, label='Direct MLM', zorder=2)

        # 用于存储当前子图中所有创建的 text 对象
        texts = []

        # 收集基线方法文本对象
        for i, txt in enumerate(labels_other):
            t = ax.text(x_other[i], y_other[i], txt, fontsize=9.5)
            texts.append(t)

        # 收集我们方法文本对象
        for i, txt in enumerate(labels_our):
            t = ax.text(x_our[i], y_our[i], txt, fontsize=9.5)
            texts.append(t)
        ...
        if dataset_name == 'NeurIPS':
            ax.set_xlim(0.2, 10.5)
        else:
            ax.set_xlim(0.2, 9.5)

        # ------ 调用 adjustText 自动调整所有文本标签位置 ------
        # expand_points=(1.5, 1.5) 用于调大点与文字间的排斥距离
        # force_points=(0.2, 0.5) 稍微加大y轴上的推力，防止水平重叠时字叠在一起
    ...
    plot_dataset(ax1, x_sind, y_sind, labels_sind, 'SIND')
    plot_dataset(ax2, x_roc, y_roc, labels_roc, 'ROCStory')
    plot_dataset(ax3, x_nips, y_nips, labels_nips, 'NeurIPS')

    # 给所有子图都加上 y 轴标签
    ax1.set_ylabel('Kendall’s Tau (τ)', fontsize=12)
    ax2.set_ylabel('Kendall’s Tau (τ)', fontsize=12)
    ax3.set_ylabel('Kendall’s Tau (τ)', fontsize=12)
...
def triple_draw_critic_strategy():
    # ==================== 1. 数据准备 ====================
    # ROCStory 数据
    labels_roc = ['Direct MLM', 'Top-3', 'Top-5', 'Top-7', 'Random-2', 'Random-3', 'Random-4']
    x_roc = [1.0, 4.0, 6.0, 8.0, 4.0, 6.0, 8.0]
    ...
    # 新增：NIPS 数据
    labels_nips = ['Direct MLM', 'Top-3', 'Top-5', 'Top-7', 'Random-2', 'Random-3', 'Random-4']
    x_nips = [1.0, 4.0, 6.0, 8.0, 4.0, 6.0, 8.0]
    y_nips = [0.782, 0.791, 0.790, 0.792, 0.793, 0.799, 0.800]
        ...
        for i, txt in enumerate(labels_data):
            ...
            if dataset_name == 'ROCStory':
                if txt == 'Top-3':
                    ...
                    xytext = (-70, 5)  # 将公共起点的文字往左移
            elif dataset_name in ['SIND', 'NIPS']:
                # NIPS 的数据趋势与 SIND 类似，Random 整体高于 Top，故将 Top 系列统一向下错开
                if txt in ['Top-3', 'Top-5', 'Top-7']:
                    xytext = (6, -14)  
                elif txt == 'Direct MLM':
                    xytext = (-70, 5)  
    ...
    # ==================== 4. 绘图与布局微调 ====================
    # 分别画左边的 SIND、中间的 ROCStory、右边的 NIPS
    plot_dataset(ax1, x_sind, y_sind, labels_sind, 'SIND')
    plot_dataset(ax2, x_roc, y_roc, labels_roc, 'ROCStory')
    plot_dataset(ax3, x_nips, y_nips, labels_nips, 'NIPS')
    ...
    # 保存为符合 LaTeX 导入规范的高清 PDF 矢量图
    plt.savefig('critic_strategies.pdf', bbox_inches='tight')
    print("具有两条策略趋势虚线的高清三子图已成功生成：'critic_strategies.pdf'")